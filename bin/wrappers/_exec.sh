...
ce_run_kimi_cli() {
    ...
    if [[ "$CE_DRY_RUN" -eq 1 ]]; then
        echo "DRY RUN - Would execute Kimi CLI:"
        echo "  ${KIMI_CLI_BIN:-kimi} --print --final-message-only --afk --work-dir $CE_CWD --model ${KIMI_MODEL:-kimi-code/kimi-for-coding} --prompt <assembled prompt>"
        echo "  KIMI_CLI_ISOLATE_SHARE=${KIMI_CLI_ISOLATE_SHARE:-1}"
        echo ""
        echo "Assembled brief: $CE_ASSEMBLED_BRIEF"
        echo "Prompt file: $prompt_file"
        return 0
    ...
    local kimi_share_dir=""
    if [[ "${KIMI_CLI_ISOLATE_SHARE:-1}" -ne 0 && -z "${KIMI_SHARE_DIR:-}" ]]; then
        kimi_share_dir="$(mktemp -d "${TMPDIR:-/tmp}/dispatch-kimi-share-XXXXXX")"
        if [[ -f "${KIMI_CLI_CONFIG_SOURCE:-$HOME/.kimi/config.toml}" ]]; then
            ln -s "${KIMI_CLI_CONFIG_SOURCE:-$HOME/.kimi/config.toml}" "$kimi_share_dir/config.toml"
        fi
        if [[ -d "${KIMI_CLI_CREDENTIALS_SOURCE:-$HOME/.kimi/credentials}" ]]; then
            ln -s "${KIMI_CLI_CREDENTIALS_SOURCE:-$HOME/.kimi/credentials}" "$kimi_share_dir/credentials"
        fi
        printf '{"mcpServers":{}}\n' > "$kimi_share_dir/mcp.json"
        mkdir -p "$kimi_share_dir/logs"
        export KIMI_SHARE_DIR="$kimi_share_dir"
    fi

    local exit_code=0
    KIMI_PROMPT_FILE="$prompt_file" \
    KIMI_CWD="$CE_CWD" \
    KIMI_READ_ONLY="$CE_READ_ONLY" \
    python3 "$CE_SCRIPT_DIR/kimi_cli_runner.py" 2>&1 | tee "$log_file" || exit_code=$?

    rm -f "$prompt_file" "$CE_ASSEMBLED_BRIEF"
    if [[ -n "$kimi_share_dir" ]]; then
        rm -rf "$kimi_share_dir"
    fi
    ...
    if [[ $exit_code -ne 0 ]]; then
        echo "Worker $CE_WORKER_ID: kimi-cli exited with code $exit_code." >&2
