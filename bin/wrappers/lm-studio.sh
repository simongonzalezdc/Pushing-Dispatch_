#!/usr/bin/env bash
# lm-studio.sh - compatibility name for the NUC mode-aware OpenAI router.
...
export OPENAI_COMPAT_BASE_URL="${OPENAI_COMPAT_BASE_URL:-${LM_STUDIO_BASE_URL:-${LMSTUDIO_BASE_URL:-${FACTORY_SELF_HOSTED_INFERENCE_URL:-${LOCAL_BASE_URL:-http://100.113.174.74:4000/v1}}}}}"
export OPENAI_COMPAT_PATH="${OPENAI_COMPAT_PATH:-/chat/completions}"
export OPENAI_COMPAT_MODEL="${OPENAI_COMPAT_MODEL:-${LM_STUDIO_MODEL:-${LMSTUDIO_MODEL:-${LOCAL_MODEL:-qwen3.5-35b-a3b}}}}"
